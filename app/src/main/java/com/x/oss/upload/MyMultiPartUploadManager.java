...
import com.aliyun.oss.ossdemo.PauseableUploadRequest;
import com.aliyun.oss.ossdemo.PauseableUploadResult;
 ...
 * 完成断点上传的任务管理
 */
public class MyMultiPartUploadManager {
    ...
    public MyMultiPartUploadManager(OSS oss, String bucket, int partSize) {
        this.oss = oss;
        this.bucket = bucket;
        this.partSize = partSize;
    }

    public PauseableUploadTask asyncUpload(String objectName, String localFileName, ExecutorService limitTaskExcutor) {
...
//                UIDisplayer.displayInfo(info);
        ...
        //利用线程池管理
        MyUploadTask myUploadTask = new MyUploadTask(localFile,bucket,object,multiPartStatus,task,partSize);
        limitTaskExcutor.execute(myUploadTask);
...
//        }).start();
    ...
    OSSProgressCallback<PauseableUploadRequest> ossProgressCallback = new OSSProgressCallback<PauseableUploadRequest>() {
        @Override
        public void onProgress(PauseableUploadRequest pauseableUploadRequest, long currentSize, long totalSize) {
            int progress = (int) (100 * currentSize / totalSize);
            EventBus.getDefault().post(new ProgressEventType(progress,pauseableUploadRequest.getLocalFile()));
